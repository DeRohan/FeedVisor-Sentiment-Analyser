...
def process_video_with_emotion_analysis(video_path, output_path):
    ...
    if frame_list:
        ...
        size = (width, height)
        output = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*"XVID"), 60, size)
...
# Streamlit app
def main():
    st.title("Emotion and Hand Analysis App")
    ...
    if selected_page == "Search":
        ...
        if search_query:
            if search_query.startswith("http://") or search_query.startswith("https://"):
                ...
                # st.page_link("./pages/Home.py", label="Analysis", icon="1️⃣")

    elif selected_page == "Emotion Analysis":
        st.write("## Emotion and Hand Analysis Page")
